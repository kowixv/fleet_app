...
import { computePM, formatPMRemaining } from "@/lib/maintenance";
...
export async function GET(req: Request) {
  ...
  const [rulesResult, settingsResult, groupsResult] = await Promise.all([
    supabase
      .from("maintenance_rules")
      .select("*, vehicles!maintenance_rules_vehicle_id_fkey(id, unit_number, current_mileage)")
      .eq("active", true),
    supabase.from("settings").select("organization_id, pm_due_soon_miles, pm_due_soon_days"),
    supabase.from("telegram_groups").select("organization_id, chat_id, vehicle_id, active"),
  ]);

  const queryError = rulesResult.error ?? settingsResult.error ?? groupsResult.error;
  if (queryError) {
    console.error("pm-check query failed", queryError);
    return Response.json({ ok: false, error: queryError.message }, { status: 500 });
  }

  const thresholdsByOrg = new Map<string, { dueSoonMiles: number; dueSoonDays: number }>();
  ...
  for (const rule of rulesResult.data ?? []) {
    ...
    if (!vehicle) continue;
    const thresholds = thresholdsByOrg.get(rule.organization_id) ?? { dueSoonMiles: 2_000, dueSoonDays: 7 };
    const pm = computePM(rule, Number(vehicle.current_mileage ?? 0), thresholds, todayISO());
    ...
    alertsMapped++;
    const line = `• Unit ${escapeHtml(vehicle.unit_number)} — ${escapeHtml(rule.service_type)}: ${escapeHtml(pm.label)} (${escapeHtml(formatPMRemaining(pm))})`;
